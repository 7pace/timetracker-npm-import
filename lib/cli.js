...
var CLI = /** @class */ (function () {
    ...
    CLI.prototype.executeCLI = function () {
        ...
        mapData.forEach(function (worklogs, organization) {
            var apiUrl = program.api ? program.api : "https://" + organization + ".timehub.7pace.com";
            var token = program.token ? program.token : config.organizationTokens[organization];
            console.log("Importing worklogs to " + apiUrl + "...");
            _this.dataService.getUserIdActivityTypeId(worklogs, apiUrl, authType, token, organization, program.user, program.password)
                .then(function () {
                _this.dataService.sendWorklogs(worklogs, apiUrl, authType, token, organization, program.user, program.password)
